...
interface ShoppingCartProps {
  items: { id: string; title: string; count: number; image: string }[];
}
...
const ShoppingCart: React.FC<ShoppingCartProps> = ({ items }) => {
  ...
  useEffect(() => {
    ...
    const storedTime = localStorage.getItem("cartTime");
    ...
    if (storedData) {
      ...
      //     setQuantities(JSON.parse(storedData));
      //   } else {
  ...
  useEffect(() => {
    localStorage.setItem("cartData", JSON.stringify(quantities));
